...
class Parametric_tSNE(object):
    ...
    def __init__(self, num_inputs, num_outputs, perplexities,
                 ...
                 do_pretrain=True, beta_batch_size=500, seed=0, epochs=10, verbose=0):
        """
        ...
        self.beta_batch_size = beta_batch_size
        self.epochs = epochs
...
#             all_layer_sizes = [num_inputs, 500, 500, 2000, num_outputs]
    ...
    def _init_model(self):
        ...
        self.encoder = models.Sequential(self._all_layers)
    ...
    def fit(self, training_data, training_betas=None):
        """
        ...
        if self._training_betas is None:
            training_betas = self._calc_training_betas(training_data, self.perplexities, beta_batch_size=self.beta_batch_size)
            self._training_betas = training_betas
        else:
...
#             self._pretrain_layers(training_data, batch_size=self._batch_size, epochs=epochs, verbose=verbose)
            self.pretrain_pca(training_data, self.epochs, self.verbose, self._batch_size, learning_rate=0.01)
        else:
            self.encoder = models.Sequential(self._all_layers)
        
        self._init_loss_func()
        self.encoder.compile(self._optimizer, self._loss_func)
...
#         X_train, y_train = self._make_train_data(training_data, self._training_betas)
        ...
        if self.verbose:
            print('{time}: Beginning training on {epochs} epochs'.format(time=datetime.datetime.now(), epochs=self.epochs))
        self.encoder.fit(train_generator, steps_per_epoch=batches_per_epoch, epochs=self.epochs, verbose=self.verbose)
#         self.encoder.fit(X_train, y_train, batch_size=self._batch_size, epochs=epochs, verbose=self.verbose)

        if self.verbose:
            print('{time}: Finished training on {epochs} epochs'.format(time=datetime.datetime.now(), epochs=self.epochs))
    ...
    def transform(self, test_data):
        ...
        assert self.encoder is not None, "Must train the model before transforming!"
        assert test_data.shape[1] == self.num_inputs, "Input test data must be same shape as training `num_inputs`"
        return self.encoder.predict(test_data)
