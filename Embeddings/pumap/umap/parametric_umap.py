...
class ParametricUMAP(UMAP):
    ...
    def _fit_embed_data(self, X, n_epochs, init, random_state):
        ...
        self.head = tf.constant(tf.expand_dims(head.astype('float32'), 0))
        self.tail = tf.constant(tf.expand_dims(tail.astype('float32'), 0))
    ...
    def save(self, save_location, verbose=True):
        ...
        if self.encoder is not None:
            encoder_output = os.path.join(save_location, "encoder_model.h5")
        ...
        if self.decoder is not None:
            decoder_output = os.path.join(save_location, "decoder.h5")
        ...
        if self.parametric_model is not None:
            parametric_model_output = os.path.join(save_location, "parametric_model.h5")
...
def construct_edge_dataset(
    ...
    if parametric_embedding:
        edge_dataset = tf.data.Dataset.from_tensor_slices(
            (edges_to_exp.astype('int32'), edges_from_exp.astype('int32'))
        )
